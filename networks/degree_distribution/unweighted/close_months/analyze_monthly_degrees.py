...
OUTPUT_DIR = os.path.join(PROJECT_ROOT, 'plots', 'degree_distributions', 'unweighted', 'close_months')
...
def process_city_degree_distributions(city):
    print(f"Processing degree distributions (close months) for: {city}...")
    ...
    for node in G_early.nodes():
        ...
        # Count neighbors only if they fall within the close_months set
        degree = sum(1 for neighbor in G_early.neighbors(node) if int(neighbor.split('-')[1]) in close_months)
    ...
    for node in G_late.nodes():
        month = int(node.split('-')[1])
        close_months = get_close_months(month)
        degree = sum(1 for neighbor in G_late.neighbors(node) if int(neighbor.split('-')[1]) in close_months)
    ...
    for month in range(1, 13):
        ...
        ax.set_xlabel('Node Degree (Close Months)', fontsize=12)
    ...
    plt.suptitle(f'Monthly Unweighted Degree Distributions (Close Months): {city}', fontsize=22, y=0.98)
    plt.tight_layout(rect=[0, 0.02, 1, 0.96])
    
    # Save the plot
    output_path = os.path.join(OUTPUT_DIR, f'{city}_monthly_degree_distribution_close_months.png')
    plt.savefig(output_path, dpi=300)
    plt.close()
    print(f"  - Saved plot to {output_path}")
    
    # Save the stats
    stats_output_path = os.path.join(STATS_DIR, f'{city}_monthly_degree_stats_close_months.json')
