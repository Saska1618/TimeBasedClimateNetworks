...
OUTPUT_DIR = os.path.join(PROJECT_ROOT, 'plots', 'degree_distributions', 'weighted', 'all_months')
...
def process_city_degree_distributions(city):
    print(f"Processing weighted degree distributions for: {city}...")
    ...
    # Extract weighted degrees (node strength) for the early period
    for node in G_early.nodes():
        # Node IDs are typically formatted as 'YYYY-MM'
        month = int(node.split('-')[1])
        early_degrees[month].append(G_early.degree(node, weight='weight'))
        
    # Extract weighted degrees (node strength) for the late period
    for node in G_late.nodes():
        month = int(node.split('-')[1])
        late_degrees[month].append(G_late.degree(node, weight='weight'))
    ...
    for month in range(1, 13):
        ...
        ax.set_xlabel('Weighted Node Degree (Strength)', fontsize=12)
    ...
    plt.suptitle(f'Monthly Weighted Degree Distributions: {city}', fontsize=22, y=0.98)
    plt.tight_layout(rect=[0, 0.02, 1, 0.96])
    
    # Save the plot
    output_path = os.path.join(OUTPUT_DIR, f'{city}_monthly_weighted_degree_distribution.png')
    plt.savefig(output_path, dpi=300)
    plt.close()
    print(f"  - Saved plot to {output_path}")
    
    # Save the stats
    stats_output_path = os.path.join(STATS_DIR, f'{city}_monthly_weighted_degree_stats.json')
