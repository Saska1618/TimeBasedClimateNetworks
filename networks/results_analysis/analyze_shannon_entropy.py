"""
Analyze the change in Shannon Entropy from the early to late period.
...
def main():
    ...
    if 'shannon_entropy' not in ds.data_vars:
        raise ValueError("'shannon_entropy' not found in the dataset variables!")

    # 2. Extract Data
    entropy = ds['shannon_entropy']
    
    # 3. Flatten valid (non-NaN) values for the distribution plots
    early_flat = entropy.sel(period='early').values.flatten()
    early_flat = early_flat[~np.isnan(early_flat)]
    
    late_flat = entropy.sel(period='late').values.flatten()
    late_flat = late_flat[~np.isnan(late_flat)]

    # 4. Calculate Differences (Late - Early) matching cell-by-cell and month-by-month
    diff_da = entropy.sel(period='late') - entropy.sel(period='early')
    ...
    sns.histplot(early_flat, color='#2563eb', label='Early (1961–1990)', kde=True, stat='density', alpha=0.5, bins=40)
    sns.histplot(late_flat, color='#ea580c', label='Late (1995–2024)', kde=True, stat='density', alpha=0.5, bins=40)
    plt.title('Shannon Entropy Distribution: Early vs. Late Period', fontsize=14, pad=15)
    plt.xlabel('Shannon Entropy', fontsize=12)
    plt.ylabel('Density', fontsize=12)
    plt.legend(frameon=True, shadow=True)
    plt.tight_layout()
    plt.savefig(output_dir / 'entropy_distribution_comparison.png', dpi=300)
    plt.close()

    # --- Plot 2: Histogram of Differences ---
    plt.figure(figsize=(10, 6))
    sns.histplot(diff_flat, color='purple', bins=50, kde=True)
    plt.axvline(0, color='black', linestyle='--', linewidth=1.5)
    plt.title('Change in Shannon Entropy (Late − Early)', fontsize=14, pad=15)
    plt.xlabel('Δ Shannon Entropy', fontsize=12)
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'entropy_change_histogram.png', dpi=300)
    plt.close()

    # --- Plot 3: Spatial Map of Mean Change ---
    mean_diff_spatial = diff_da.mean(dim='month')
    plt.figure(figsize=(10, 8))
    mean_diff_spatial.plot(cmap='RdBu_r', center=0, cbar_kwargs={'label': 'Mean Δ Shannon Entropy', 'shrink': 0.8})
    plt.title('Spatial Distribution: Mean Change in Shannon Entropy\n(Averaged across all months)', fontsize=14, pad=15)
    plt.tight_layout()
    plt.savefig(output_dir / 'entropy_change_spatial_map.png', dpi=300)
    plt.close()
    ...
    for i, month_val in enumerate(diff_da.month.values):
        ...
        sns.histplot(month_data, ax=axes[i], color='teal', bins=30, kde=True)
        axes[i].axvline(0, color='black', linestyle='--', linewidth=1.2)
        axes[i].set_title(month_names[i], fontsize=12)
        
        # Clean up the axis labels for the grid
        if i >= 8:  # Bottom row
            axes[i].set_xlabel('Δ Shannon Entropy', fontsize=10)
        else:
            axes[i].set_xlabel('')
    ...
    plt.suptitle('Monthly Change in Shannon Entropy (Late − Early)', fontsize=16, y=0.98)
    plt.tight_layout()
    plt.savefig(output_dir / 'entropy_change_per_month_histograms.png', dpi=300)
    plt.close()
