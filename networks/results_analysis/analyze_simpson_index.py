"""
Analyze the change in Simpson Index from the early to late period.
...
def main():
    ...
    if 'simpson_index' not in ds.data_vars:
        raise ValueError("'simpson_index' not found in the dataset variables!")

    # 2. Extract Data
    index_data = ds['simpson_index']
    ...
    sns.histplot(early_flat, color='#059669', label='Early (1961–1990)', kde=True, stat='density', alpha=0.5, bins=40)
    sns.histplot(late_flat, color='#7c3aed', label='Late (1995–2024)', kde=True, stat='density', alpha=0.5, bins=40)
    plt.title('Simpson Index Distribution: Early vs. Late Period', fontsize=14, pad=15)
    plt.xlabel('Simpson Index', fontsize=12)
    plt.ylabel('Density', fontsize=12)
    plt.legend(frameon=True, shadow=True)
    plt.tight_layout()
    plt.savefig(output_dir / 'simpson_distribution_comparison.png', dpi=300)
    plt.close()

    # --- Plot 2: Histogram of Differences ---
    plt.figure(figsize=(10, 6))
    sns.histplot(diff_flat, color='indianred', bins=50, kde=True)
    plt.axvline(0, color='black', linestyle='--', linewidth=1.5)
    plt.title('Change in Simpson Index (Late − Early)', fontsize=14, pad=15)
    plt.xlabel('Δ Simpson Index', fontsize=12)
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'simpson_change_histogram.png', dpi=300)
    plt.close()

    # --- Plot 3: Spatial Map of Mean Change ---
    mean_diff_spatial = diff_da.mean(dim='month')
    plt.figure(figsize=(10, 8))
    mean_diff_spatial.plot(cmap='BrBG', center=0, cbar_kwargs={'label': 'Mean Δ Simpson Index', 'shrink': 0.8})
    plt.title('Spatial Distribution: Mean Change in Simpson Index\n(Averaged across all months)', fontsize=14, pad=15)
    plt.tight_layout()
    plt.savefig(output_dir / 'simpson_change_spatial_map.png', dpi=300)
    plt.close()
    ...
    for i, month_val in enumerate(diff_da.month.values):
        ...
        sns.histplot(month_data, ax=axes[i], color='steelblue', bins=30, kde=True)
        axes[i].axvline(0, color='black', linestyle='--', linewidth=1.2)
        axes[i].set_title(month_names[i], fontsize=12)
        
        if i >= 8: axes[i].set_xlabel('Δ Simpson Index', fontsize=10)
        else: axes[i].set_xlabel('')
        if i % 4 == 0: axes[i].set_ylabel('Count', fontsize=10)
        else: axes[i].set_ylabel('')

    plt.suptitle('Monthly Change in Simpson Index (Late − Early)', fontsize=16, y=0.98)
    plt.tight_layout()
    plt.savefig(output_dir / 'simpson_change_per_month_histograms.png', dpi=300)
    plt.close()
