"""
Analyze the change in Number of Communities from the early to late period.
...
def main():
    ...
    if 'num_communities' not in ds.data_vars:
        raise ValueError("'num_communities' not found in the dataset variables!")

    # 2. Extract Data
    index_data = ds['num_communities']
    ...
    sns.histplot(early_flat, color='#d97706', label='Early (1961–1990)', kde=True, stat='density', alpha=0.5, discrete=True)
    sns.histplot(late_flat, color='#4f46e5', label='Late (1995–2024)', kde=True, stat='density', alpha=0.5, discrete=True)
    plt.title('Number of Communities Distribution: Early vs. Late Period', fontsize=14, pad=15)
    plt.xlabel('Number of Communities', fontsize=12)
    plt.ylabel('Density', fontsize=12)
    plt.legend(frameon=True, shadow=True)
    plt.tight_layout()
    plt.savefig(output_dir / 'num_communities_distribution_comparison.png', dpi=300)
    plt.close()

    # --- Plot 2: Histogram of Differences ---
    plt.figure(figsize=(10, 6))
    sns.histplot(diff_flat, color='seagreen', discrete=True, kde=True)
    plt.axvline(0, color='black', linestyle='--', linewidth=1.5)
    plt.title('Change in Number of Communities (Late − Early)', fontsize=14, pad=15)
    plt.xlabel('Δ Number of Communities', fontsize=12)
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'num_communities_change_histogram.png', dpi=300)
    plt.close()

    # --- Plot 3: Spatial Map of Mean Change ---
    mean_diff_spatial = diff_da.mean(dim='month')
    plt.figure(figsize=(10, 8))
    mean_diff_spatial.plot(cmap='PRGn', center=0, cbar_kwargs={'label': 'Mean Δ Number of Communities', 'shrink': 0.8})
    plt.title('Spatial Distribution: Mean Change in Number of Communities\n(Averaged across all months)', fontsize=14, pad=15)
    plt.tight_layout()
    plt.savefig(output_dir / 'num_communities_change_spatial_map.png', dpi=300)
    plt.close()
    ...
    for i, month_val in enumerate(diff_da.month.values):
        ...
        sns.histplot(month_data, ax=axes[i], color='olivedrab', discrete=True, kde=True)
        axes[i].axvline(0, color='black', linestyle='--', linewidth=1.2)
        axes[i].set_title(month_names[i], fontsize=12)
        
        if i >= 8: axes[i].set_xlabel('Δ Number of Communities', fontsize=10)
        else: axes[i].set_xlabel('')
        if i % 4 == 0: axes[i].set_ylabel('Count', fontsize=10)
        else: axes[i].set_ylabel('')

    plt.suptitle('Monthly Change in Number of Communities (Late − Early)', fontsize=16, y=0.98)
    plt.tight_layout()
    plt.savefig(output_dir / 'num_communities_change_per_month_histograms.png', dpi=300)
    plt.close()
